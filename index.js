 import { initializeApp } from "https://www.gstatic.com/firebasejs/12.7.0/firebase-app.js";
        ...
        const docRef = doc(db, "itinerary", "gIN2cSCVSD7OGjBVHU9x");
        ...
        window.addItem = async function(type) {
            if(type === 'shopping') {
                ...
                const title = document.getElementById('exp-title'), 
                      ...
                      currency = document.getElementById('exp-currency'),
        ...
        function renderExpense() {
            ...
            container.innerHTML = expenseData.map(item => {
                const symbol = item.currency === 'JPY' ? '¥' : '$';
                if(item.currency === 'JPY') totalJPY += item.amount;
                else totalTWD += item.amount;
                ...
                <div class="list-item">
                    <div class="flex-1">
                        ...
                        <div class="flex gap-2 mt-1">
                            <span class="expense-badge">${item.method}</span>
                            <span class="expense-badge text-blue-500">${item.user}</span>
                        </div>
                    </div>
                    <div class="text-right">
                        <div class="font-black ${item.currency === 'JPY' ? 'text-blue-600' : 'text-green-600'}">
            ...
            document.getElementById('total-twd').innerText = `$ ${totalTWD.toLocaleString()}`;
            lucide.createIcons();
        }
